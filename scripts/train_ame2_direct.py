...
parser.add_argument("--robot",            type=str,   default="anymal_d",
                    choices=["anymal_d", "tron1"],
                    help="Robot to train: anymal_d (12-DOF quadruped) or tron1 (6-DOF biped)")
...
from ame2_direct.env import make_obs_layout, pack_obs, unpack_obs  # noqa: E402

try:
    from ame2.networks.rslrl_wrapper import AME2ActorCritic
    from ame2.networks.ame2_model import anymal_d_policy_cfg, tron1_policy_cfg
    from ame2.robot_configs import ANYMAL_D_ROBOT, TRON1_ROBOT
except ImportError:
    from robot_lab.tasks.manager_based.locomotion.ame2.networks.rslrl_wrapper import AME2ActorCritic
    from robot_lab.tasks.manager_based.locomotion.ame2.networks.ame2_model import anymal_d_policy_cfg, tron1_policy_cfg
    from robot_lab.tasks.manager_based.locomotion.ame2.robot_configs import ANYMAL_D_ROBOT, TRON1_ROBOT
...
def main():
    ...
    # ── Robot dispatch ──
    if args_cli.robot == "tron1":
        from ame2_direct.config_tron1 import TRON1DirectEnvCfg
        from ame2_direct.env_tron1 import TRON1DirectEnv

        policy_cfg = tron1_policy_cfg()
        policy_cfg.d_prop_critic = TRON1_ROBOT.d_prop_critic_ext  # 37
        robot_cfg = TRON1_ROBOT
        env_cfg = TRON1DirectEnvCfg()
        num_actions = 6
        d_contact = robot_cfg.num_contacts  # 7

        # Build TRON1 obs layout for the wrapper
        _tron1_slices, _tron1_flat_dim, _tron1_map_h, _tron1_map_w = make_obs_layout(
            d_prop=robot_cfg.d_prop_raw,  # 30
            map_h=policy_cfg.map_h,       # 13
            map_w=policy_cfg.map_w,       # 18
            d_critic=robot_cfg.d_prop_critic_ext,  # 37
            d_contact=robot_cfg.num_contacts,      # 7
        )
    else:
        policy_cfg = anymal_d_policy_cfg()
        policy_cfg.d_prop_critic = ANYMAL_D_ROBOT.d_prop_critic_ext  # 55
        robot_cfg = ANYMAL_D_ROBOT
        env_cfg = AME2DirectEnvCfg()
        num_actions = 12
        d_contact = robot_cfg.num_contacts  # 13
        _tron1_slices = None
        _tron1_flat_dim = None
        _tron1_map_h = None
        _tron1_map_w = None
    ...
    print(f"[Rank {rank}] Creating env ({args_cli.num_envs} envs, robot={args_cli.robot})...", flush=True)

    if args_cli.robot == "tron1":
        env = TRON1DirectEnv(cfg=env_cfg)
    else:
        env = AME2DirectEnv(cfg=env_cfg)
    print(f"[Rank {rank}] Env created", flush=True)

    # Build wrapper with correct obs layout
    if _tron1_slices is not None:
        # TRON1: custom obs layout
        class TRON1DirectWrapper(AME2DirectWrapper):
            """AME2DirectWrapper with TRON1 obs layout."""
            ...
            def get_observations(self):
                if self._last_obs is None:
                    ...
                    self._last_obs = pack_obs(obs_dict, self.num_envs,
                                              slices=_tron1_slices, flat_dim=_tron1_flat_dim)
                extras = self.extras
        ...
        rsl_env = TRON1DirectWrapper(env, device=device)
    else:
    ...
    ame2_net = AME2ActorCritic(
        ...
        is_student=False,
        robot_cfg=robot_cfg,
        critic_kwargs={"d_contact": d_contact},
    ).to(device)
    ...
    if is_main:
        ...
        print(f"\n[AME-2 Teacher — True Distributed — {args_cli.robot}]")
        print(f"  Robot: {args_cli.robot}  joints={num_actions}  contacts={d_contact}")
    ...
    if is_main:
        ...
        wandb.init(
            ...
            name=f"{args_cli.robot}-teacher-8xH100-truedist",
            config={
