...
from ..robot_configs import RobotConfig, ANYMAL_D_ROBOT, TRON1_ROBOT
...
# ---------------------------------------------------------------------------
# Now parametrized via RobotConfig (ame2.robot_configs) for multi-robot support.
# Legacy ANYmal-D hardcoded constants kept as module-level aliases for
# backward compatibility with standalone tests.
_LR_JOINT_PERM   = ANYMAL_D_ROBOT.lr_joint_perm
_LR_JOINT_SIGN   = ANYMAL_D_ROBOT.lr_joint_sign
_LR_CONTACT_PERM = ANYMAL_D_ROBOT.lr_contact_perm
...
def _flip_lr(
    ...
    Args:
        map_feat:   (B, C, H, W)  — flip the H (lateral/y) axis.
        prop:       (B, d_prop)   — negate lateral dims, permute joint order.
        contact:    (B, d_contact) — swap left/right contacts.
        is_teacher: True  → teacher prop layout: base_vel(3)|ang_vel(3)|grav(3)|q(J)|dq(J)|act(J)|cmd
                    False → student prop layout:             ang_vel(3)|grav(3)|q(J)|dq(J)|act(J)|cmd
        robot_cfg:  RobotConfig with symmetry info. Defaults to ANYMAL_D_ROBOT.

    Returns:
        (map_f, prop_f, contact_f) — mirrored tensors, same device/dtype.
    """
    if robot_cfg is None:
        robot_cfg = ANYMAL_D_ROBOT

    J = robot_cfg.num_joints
    joint_perm = robot_cfg.lr_joint_perm
    joint_sign_list = robot_cfg.lr_joint_sign
    contact_perm = robot_cfg.lr_contact_perm
    ...
    prop_f = prop.clone()
    sign = torch.tensor(joint_sign_list, device=prop.device, dtype=prop.dtype)

    if is_teacher:
        # Teacher layout: base_vel(3)|ang_vel(3)|grav(3)|q(J)|dq(J)|act(J)|cmd(...)
        # Base offsets: q starts at 9, dq at 9+J, act at 9+2J, cmd at 9+3J
        q_start = 9
        prop_f[:, 1]  *= -1.0   # base_lin_vel y
        prop_f[:, 3]  *= -1.0   # ang_vel roll (wx)
        prop_f[:, 5]  *= -1.0   # ang_vel yaw  (wz)
        prop_f[:, 7]  *= -1.0   # gravity y
        for offset in (q_start, q_start + J, q_start + 2 * J):
            prop_f[:, offset:offset + J] = prop_f[:, offset:offset + J][:, joint_perm] * sign

        d = prop.shape[-1]
        base_end = q_start + 3 * J  # = 9 + 3J (where cmd starts)
        d_prop_raw = robot_cfg.d_prop_raw           # 3 + d_hist + 3
        d_prop_critic = robot_cfg.d_prop_critic      # 3 + d_hist + 5
        d_prop_critic_ext = robot_cfg.d_prop_critic_ext  # d_prop_critic + 5

        if d == d_prop_raw:
            # Actor prop — cmd layout: [base_end]=d_xy, [base_end+1]=sin(yaw), [base_end+2]=cos(yaw)
            prop_f[:, base_end + 1] *= -1.0   # sin(yaw_rel) — odd
        elif d == d_prop_critic:
            # Critic prop — cmd: [base_end]=x_rel, [+1]=y_rel, [+2]=sin, [+3]=cos, [+4]=t_rem
            prop_f[:, base_end + 1] *= -1.0   # y_rel — lateral
            prop_f[:, base_end + 2] *= -1.0   # sin(yaw_rel) — odd
        elif d == d_prop_critic_ext:
            # Extended critic: critic_prop + nav_extra(5D)
            prop_f[:, base_end + 1] *= -1.0   # y_rel
            prop_f[:, base_end + 2] *= -1.0   # sin(yaw_rel)
            prop_f[:, d - 1] *= -1.0           # world_vel_y (last element of nav_extra)
        else:
            raise ValueError(
                f"_flip_lr is_teacher=True: unexpected prop dim {d}, "
                f"expected {d_prop_raw} (actor) or {d_prop_critic}/{d_prop_critic_ext} (critic). "
                f"Robot: {robot_cfg.name}, J={J}"
            )
    else:
        # Student layout: ang_vel(3)|grav(3)|q(J)|dq(J)|act(J)|cmd(3)
        q_start = 6
        prop_f[:, 0]  *= -1.0   # ang_vel roll (wx)
        prop_f[:, 2]  *= -1.0   # ang_vel yaw  (wz)
        prop_f[:, 4]  *= -1.0   # gravity y
        for offset in (q_start, q_start + J, q_start + 2 * J):
            prop_f[:, offset:offset + J] = prop_f[:, offset:offset + J][:, joint_perm] * sign
        cmd_start = q_start + 3 * J  # = 6 + 3J
        prop_f[:, cmd_start + 1] *= -1.0   # sin(yaw_rel)

    # Contact: swap L/R
    contact_f = contact[:, contact_perm]
...
class AME2ActorCritic(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        Args:
            ...
            num_actions:    Must equal cfg.num_joints.
            ...
            robot_cfg:      RobotConfig for symmetry augmentation. Defaults to ANYMAL_D_ROBOT.
        ...
        self.robot_cfg = robot_cfg if robot_cfg is not None else ANYMAL_D_ROBOT
    ...
    def _unpack_obs(self, obs):
        ...
        if isinstance(obs, torch.Tensor) and obs.dim() == 2:
            from ame2_direct.env import unpack_obs, make_obs_layout
            # Use robot-specific layout if not ANYmal-D defaults
            rc = self.robot_cfg
            if rc.num_joints != 12:
                slices, _, mh, mw = make_obs_layout(
                    d_prop=rc.d_prop_raw,
                    map_h=self.cfg.map_h,
                    map_w=self.cfg.map_w,
                    d_critic=rc.d_prop_critic_ext,
                    d_contact=rc.num_contacts,
                )
                return unpack_obs(obs, slices=slices, map_h=mh, map_w=mw)
    ...
    def evaluate(self, obs, **kwargs: dict[str, Any]) -> torch.Tensor:
        ...
        map_f, prop_f, cont_f = _flip_lr(
            map_t, prop, contact, is_teacher=True,
...
def make_ame2_rslrl_agent(
    ...
    **kwargs,
    ...
    Args:
        ...
        num_actions: Number of action dims.
        stage:       "teacher" or "student".
        ame2_cfg:    PolicyConfig. Defaults to ANYmal-D.
        robot_cfg:   RobotConfig for symmetry. Defaults to ANYMAL_D_ROBOT.
...
class AME2MapEnvWrapper:
    ...
    # Default scan dims (ANYmal-D); overridden via mapping_cfg param
    SCAN_H: int = 31
    SCAN_W: int = 51
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            mapping_cfg:  MappingConfig for SCAN_H/W. Defaults to ANYmal-D (31x51).
            is_student:   If True, builds student obs (adds history + commands).
            device:       Torch device string.
        """
        if mapping_cfg is not None:
            self.SCAN_H = mapping_cfg.map_h
            self.SCAN_W = mapping_cfg.map_w
