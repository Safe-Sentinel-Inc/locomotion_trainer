...
def make_obs_layout(d_prop: int, map_h: int, map_w: int,
                    d_critic: int, d_contact: int):
    """Compute obs layout slices and total flat dimension.

    Args:
        d_prop:    Actor prop dim (48 for ANYmal-D, 30 for TRON1).
        map_h:     Policy map height (14 for ANYmal-D, 13 for TRON1).
        map_w:     Policy map width  (36 for ANYmal-D, 18 for TRON1).
        d_critic:  Critic prop dim (55 for ANYmal-D, 37 for TRON1).
        d_contact: Contact dim (13 for ANYmal-D, 7 for TRON1).

    Returns:
        (slices_dict, total_dim, map_h, map_w)
    """
    map_flat = 3 * map_h * map_w
    c = 0
    slices = {}
    slices["prop"]        = (c, c + d_prop);          c += d_prop
    slices["map"]         = (c, c + map_flat);        c += map_flat
    slices["critic_prop"] = (c, c + d_critic);        c += d_critic
    slices["map_teacher"] = (c, c + map_flat);        c += map_flat
    slices["contact"]     = (c, c + d_contact);       c += d_contact
    return slices, c, map_h, map_w


# ANYmal-D defaults (backward compatible)
_OBS_SLICES, OBS_FLAT_DIM, _OBS_MAP_H, _OBS_MAP_W = make_obs_layout(
    d_prop=48, map_h=14, map_w=36, d_critic=55, d_contact=13,
)  # OBS_FLAT_DIM = 3140


def pack_obs(obs_dict: dict, num_envs: int,
             slices: dict | None = None, flat_dim: int | None = None) -> torch.Tensor:
    """Pack structured obs dict into flat (N, D) tensor.

    Args:
        obs_dict:  Dict with keys "prop", "map", "critic_prop", "map_teacher", "contact".
        num_envs:  Batch size.
        slices:    Custom layout slices (from make_obs_layout). Defaults to ANYmal-D.
        flat_dim:  Total flat dim. Defaults to ANYmal-D (3140).
    """
    if slices is None:
        slices = _OBS_SLICES
    if flat_dim is None:
        flat_dim = OBS_FLAT_DIM
    flat = torch.empty(num_envs, flat_dim, device=obs_dict["prop"].device)
    for key, (lo, hi) in slices.items():
        flat[:, lo:hi] = obs_dict[key].reshape(num_envs, -1)
    return flat


def unpack_obs(flat: torch.Tensor,
               slices: dict | None = None,
               map_h: int | None = None,
               map_w: int | None = None) -> dict:
    """Unpack flat (N, D) tensor back into structured obs dict.

    Args:
        flat:    (N, D) tensor.
        slices:  Custom layout slices. Defaults to ANYmal-D.
        map_h:   Policy map height for reshape. Defaults to ANYmal-D (14).
        map_w:   Policy map width for reshape. Defaults to ANYmal-D (36).
    """
    if slices is None:
        slices = _OBS_SLICES
    if map_h is None:
    ...
    for key, (lo, hi) in slices.items():
        ...
        if key in ("map", "map_teacher"):
            d[key] = chunk.reshape(N, 3, map_h, map_w)
        else:
            d[key] = chunk
